...
namespace BLINK.RPGBuilder.Character
{
    public class PlayerAppearanceHandler : MonoBehaviour
    {
	    ...
	    public Transform OneHandWeapon1CombatSlot, OneHandWeapon1RestSlot, OneHandWeapon2CombatSlot, OneHandWeapon2RestSlot, TwoHandWeaponCombatSlot, TwoHandWeaponRestSlot, ShieldRestSlot, ShieldCombatSlot,BowCombatSlot,BowRestSlot;
        ...
        public void UpdateWeaponStates(bool inCombat)
        {
            if (weapon1GO != null)
            {
                if (inCombat)
                {
                    ...
                    else if (weapon1RPGItem.weaponType == "BOW")
	                {
	                    weapon1GO.transform.SetParent(BowCombatSlot);
	                }
                ...
                else
                {
	                ...
	                {
	                    weapon1GO.transform.SetParent(BowRestSlot);
	                }
                    else
                    {
                        weapon1GO.transform.SetParent(weapon1RPGItem.slotType == "TWO HAND"
            ...
            if (weapon2GO != null)
            {
                ...
                else if (weapon2RPGItem.weaponType == "BOW")
	            {
	                weapon2GO.transform.SetParent(inCombat ? BowCombatSlot : BowRestSlot);
	            }
        ...
        private void ParentWeaponToSlot(GameObject weaponGO, int weaponID, string slotType, RPGItem weaponItem)
        {
            if (CombatManager.playerCombatNode == null)
            {
                if (weaponID == 1)
                {
                    ...
                    else if (weaponItem.weaponType == "BOW")
	                {
	                    weaponGO.transform.SetParent(BowRestSlot);
	                }
                    else
                    {
                ...
                else
                {
	                ...
	                {
		                weaponGO.transform.SetParent(weaponItem.weaponType == "BOW"
			                ? BowRestSlot
			                : OneHandWeapon2RestSlot);
	                }
